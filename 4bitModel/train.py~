...
from models.quant.quant_layer import clamp_linear,clamp_conv2d
...
parser.add_argument('--quant_file', type=str, default=None,
                    help='path to Quantization file')
...
def main():
    ...
    if args.offset_noise:
        models.quant.quant_layer.offsetnoise = args.offset_noise
    if args.bitline_noise:
        models.quant.quant_layer.bitlinenoise = args.bitline_noise
    if args.quant_file:
        quant_data = sio.loadmat(args.quant_file)
        models.quant.quant_layer.levels = torch.tensor(quant_data['levels'].astype('float32')).cuda().squeeze()
        models.quant.quant_layer.edges = torch.tensor(quant_data['edges'].astype('float32')).cuda().squeeze()
