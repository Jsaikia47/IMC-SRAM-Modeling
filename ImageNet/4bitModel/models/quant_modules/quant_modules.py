...
class QuantizedConv2d(nn.Conv2d):
    def __init__(self, *kargs, **kwargs):
        ...
        # quantizer
        self.activation_quantizer = PACT_Quant(n_bits=self.a_bits, alpha=self.alpha)
        self.weight_quantizer = SAWB_Quant(n_bits=self.w_bits, alpha_w=self.alpha_w)

    def forward(self, input):
        if self.w_bits == 2:
            alpha_w = get_scale_2bit(self.weight)   # update quantization boundary
        elif self.w_bits == 4:
            alpha_w = self.k * self.weight.abs().mean()               
        ...
        if self.w_bits == 2:
            ...
            base_w = 2.**(self.w_bits-1) / (2 ** (self.w_bits) - 2.) * (2 * alpha_w)
                            ...
                            for iw in range(self.w_bits):
                                ...
                                if self.mode == 'software' and (self.cdf_table_9 is not None or self.noise_9_std > 0):
                                    ...
                                    if self.noise_9_std > 0:
                                        ...
                                        out += Quant_prob(out_temp, cdf_table=self.cdf_table_9, H=45., levels=91, mode=self.mode) * (2**(-ii-iw)) * torch.sign(torch.tensor(iw-0.5)) * self.base
                                else:
                                    out += out_temp * (2**(-ii-iw)) * torch.sign(torch.tensor(iw-0.5)) * self.base
...
class QuantizedLinear(nn.Linear):
    def __init__(self, in_channels, out_channels, bias=True, a_bits=2, w_bits=2, alpha_init=10.0):
        ...
        # quantizer
        self.activation_quantizer = PACT_Quant(n_bits=a_bits, alpha=self.alpha)
        self.weight_quantizer = SAWB_Quant(n_bits=w_bits, alpha_w=self.alpha_w)
    ...
    def forward(self, input):
        if self.w_bits == 2:
            self.alpha_w.data = get_scale_2bit(self.weight)
        elif self.w_bits == 4:
            self.alpha_w.data = self.k * self.weight.abs().mean() 

        self.weight_quantizer.update_alpha(self.alpha_w)
